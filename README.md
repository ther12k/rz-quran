...
> **Status: M0–M4 implemented and tested; acceptance reconciliation complete; M5 operational execution and public launch blocked on human approvals.** This repository contains no deployed product, no licensed recitation audio, no approved curriculum and no legal sign-off. Learning content is clearly marked non-production pilot data (Hijaiyah letters + Tanzil simple-clean text for 5 short surahs, seeded locally; no audio). **Public launch stays blocked** until content-rights, curriculum and privacy approvals are recorded (see `docs/11_DECISIONS_AND_OPEN_QUESTIONS.md`). Nothing in this repo should be read as a claim of educational efficacy or legal compliance.

### Status vocabulary (used throughout this repo)

Per-criterion record: **[`docs/m5/ACCEPTANCE_MATRIX.md`](docs/m5/ACCEPTANCE_MATRIX.md)** (all 74 tasks, generated by the v1.1 acceptance-reconciliation pass).

- **Implemented** — code exists and is inspected; not a claim of acceptance.
- **Tested** — a real executed check (test/drill/measurement) with recorded evidence; never a mock or skipped check.
- **Externally approved** — a human decision with evidence (rights, curriculum, privacy, artwork, pilot, release). **None exist yet; none are fabricated.**
- **Blocked remainder** — scoped work that cannot proceed without the above (e.g. T069 operational drills need a release environment; T068 regression needs pilot findings; T012/T029/T041/T055 have documented local engineering remainders with reopen proposals pending product-owner authorization).
...
| M5 Pilot & launch | 1 closed / 7 open | Preflight tooling closed (T069 — reconciliation proposes reopening: operational execution blocked). Open: T063–T068, T070 — external approvals (rights, curriculum, privacy, artwork), the supervised usability pilot, pilot-dependent remediation (T068, engineering/QA — not a human-only gate) and release sign-off. Reconciliation also proposes reopening T012/T029/T041/T055 with scoped local engineering remainders |

Full regression (all green, post-reconciliation): typecheck · 9 contracts · 7 unit · 13 integration · 17 security (leak scans + runtime response shapes) · web build · Playwright suites incl. first-party-only outbound behavior. Performance evidence: `evidence/PERFORMANCE_REPORT.md`; restore drills (incl. independent-ledger replay): `evidence/restore-drill-*.md`; UI-vs-mockup delta: `evidence/mockup-compare/COMPARISON.md`; acceptance matrix: `docs/m5/ACCEPTANCE_MATRIX.md`.
